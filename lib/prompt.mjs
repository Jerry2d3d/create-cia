...
export function createPrompter({ input = process.stdin, output = process.stdout } = {}) {
  ...
  function ensure() {
    ...
    // Order matters on Node 24: mark ended and drop the interface BEFORE
    // resolving waiters, so a waiter never touches a closing readline
    // (pause() after close throws ERR_USE_AFTER_CLOSE there; 20/22 ignored it).
    const end = () => { ended = true; const r = rl; rl = null; while (waiters.length) waiters.shift()(CANCEL); if (r) r.close(); };
    rl.on("close", end);
    rl.on("SIGINT", end);
  }
  ...
  function ask(prompt) {
    ...
    rl.resume();
    return new Promise((resolve) => waiters.push((v) => { if (rl && !ended) rl.pause(); echoNl(); resolve(v); }));
  }
