...
export async function main(argv = process.argv.slice(2)) {
  ...
  // Prompts run on a TTY, or whenever CREATE_CIA_INTERACTIVE=1 — the answers
  // are plain lines (Enter / y / n / a number), so piping answers works too.
  // --yes, CI=1 or a bare non-TTY stdin → defaults, no prompts.
  const interactive = !args.yes && !process.env.CI && (Boolean(process.stdin.isTTY) || process.env.CREATE_CIA_INTERACTIVE === "1");
